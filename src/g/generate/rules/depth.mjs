...
export async function g_generate_rules_depth(rules, depth, skip, for_each_generated) {
  await m_js_arguments_assert(list_is, m_js_number_is, m_js_function_is, m_js_function_is)(arguments);
  if (depth === 0) {
  ...
  await g_generate_rules(rules, async rules => {
    ...
    await g_generate_rules_depth(rules, depth - 1, skip, for_each_generated);
  });
}
